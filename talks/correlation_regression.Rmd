...
One of the handiest packages in the R regression universe is `emmeans`, which can provide the 'expected marginal means' (emm), as well as a host of other contrasts and comparisons. In particular, it is very easy to test simple slopes and pairwise differences. Furthermore, the package works with `multcomp` to handle correction for multiple comparisons. See the longer documentation [here](https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html).

Let's look at the concentration of leucine in a stuy of pigs who were fed differing levels of protein in the diet (9, 12, 15, and 18%) and different protein sources (fish, soybean, milk).
...
If we wanted to compare the pairwise differences in the effect of protein source while controlling for protein percent (and potentially other variables in the model), we could use the `pairs` function:
...
Contrasts for the predicted mean level of leucine contrast for each protein source, controlling for percentage.
```{r}
pigs.emm.s@linfct
```
...
We can also get orthogonal polynomial contrasts for free in `emmeans`. For example, is the effect of protein percent linearly related to leucine, or might it be quadratic or cubic?
...
Returning to the iris dataset (from the parallel R examples), consider this:
...
In a simple slopes test, we might wish to know whether the slope for `Petal.Length` is non-zero in each species individually. Let's start by getting the estimated marginal means for each species.
...
Is the Petal.Width slope non-zero in each Species? The 'infer' argument requests t-tests and p-values for the slopes.

```{r}
summary(emtrends(model = fitiris, ~Species, var="Petal.Width"), infer=TRUE)
```

Are the slopes different by Species?
