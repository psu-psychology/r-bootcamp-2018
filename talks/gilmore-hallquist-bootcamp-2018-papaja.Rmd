...
The survey can be found at this URL: <https://docs.google.com/spreadsheets/d/1-YB0iWUNN_9oxBhz221NFiyBOcwMfHziFeUiUvQwn7k/edit>.
...
As indicated in Table \@ref(tab:apa-corr-table), there is a negative correlation (`r corr_Banjo_age$estimate`) between banjo music enthusiasm and age (`r corr_Banjo_age$statistic`), a negative correlation (`r corr_Banjo_sleep$estimate`) between banjo music enthusiasm and sleep (`r corr_Banjo_sleep$statistic`), but no correlation (`r corr_age_sleep$estimate`) between age and sleep (`r corr_age_sleep$statistic`). Figures \@ref(fig:Banjo-by-age-exp) and \@ref(fig:Banjo-by-sleep) depict these patterns.

To test the hypothesis that banjo music enthusiasm varies as a function of R expertise, we carried out a one-way ANOVA.
R experience (`r Banjo_aov_results$full$R_exp`) did not predict enthusiasm for banjo music. Table \@ref(tab:Banjo-aov-table) summarizes these results.
