...
## From single commands to sequences
...
Now, type in two additional rows of commands plus a comment.
```{r}
# Plot histograms
hist(data_mean_zero)
...
Enter `source('bootcamp_2018_08_16.R')` but do not press enter yet.
...
```{r}
# Generate 0-centered, and non-zero-centered normally distributed data
(data_mean_zero <- rnorm(n = samples_1, mean = mean_1, sd = sd_1))
(data_mean_nonzero <- rnorm(n = samples_2, mean = mean_2, sd = sd_2))
```
