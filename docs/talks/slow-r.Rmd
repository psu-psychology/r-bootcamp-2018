...
I've taken exactly two programming classes in my life.
The rest I've taught myself.
So, I'm definitely a half-fast programmer.
But, I think this means that I haven't completely forgotten what it means to pound your head against the wall trying to understand why the computer won't do what you want it to do.

Learning a programming language is lots of fun, and it can provide you a structured way for doing certain kinds of thinking.
As much as possible, I want us to make this an environment where no question (about R) is out of bounds.
...
In brief, I enter [https://lxclusterapps.tlt.psu.edu:8788](https://lxclusterapps.tlt.psu.edu:8788) in my browser, enter my PSU Access ID (rog1) and password, then click on the the `Sign In` button with my mouse or press return on my keyboard.
...
Why did typing `license()` work but typing `'license()'` give us something odd?
...
You already know about the cursor from your experiences in other computer programs.
...
Talking to computers via the console is powerful and useful.
And the console is our window into the computer's brain.
...
To save typing.
If you were writing your own command to print your name you could certainly use `print_my_name()` to do so.
Of course, you'd have to create a command, or really a function, that told R what to do when you gave this command.
We'll talk about writing our own functions later.
...
The 'verb' is first, and the 'noun(s)' come inside the parentheses.
...
But it's a good idea to train your fingers to type it the easy way.
...
In other words, I could say `my_age = 55` and R would understand it.

> Don't use `=`. You can, but don't. Use `<-`.

Why?
It's a recommendation about style, not substance.
But style matters in many important things in life, and R is no exception.
...
> Avoid flame wars unless you want to get burned.

### Asking questions

We can also ask R questions.
By typing 'comparison operators' (`==`, `!=`, `>`, `<`, etc.), commands that tell R to compare this thing to that, we can ask R true/false questions.
...
This is different from what you learned in math class, where a single equal sign like $a=1$ could be either a statement (or assignment command) or a question.
If computers had invented math, rather than people, they would probably have separated making statements or commands from asking questions--for clarity.
So, if we want to ask R a question about whether two things are equal, we use the double equal sign `==` to make it absolutely clear what we mean.
...
So, used this way, R is a very big pocket calculator.
...
There are rules about which operations R does in what order.
They are called 'rules of precedence', but we won't go over them here.
My advice is never make an expression where you aren't absolutely clear what operation will happen in what order.
...
I say *most* of the time because R sometimes treats the numbers 1 and 0 as the equivalent of `TRUE` and `FALSE`.
There is a good reason for this we'll talk about soon.

Of course, we can also talk about *commands*, so `help('sum')` tells R to give us helpful information about the `sum()` command.
...
Here, we don't need the parentheses `()` because we just information about the command itself.
...
These are the basic building blocks for interacting with R:
...
If you are working on your own computer and haven't already, install `swirl`.
...
Then choose `1: R Programming` by entering the number `1` and pressing `return` or `enter`.
Then choose `1: Basic Building Blocks` by entering the number `1` and pressing `return` or `enter`.
...
The 1's and 0's are called binary digits or bits for short.
So, the computer needs to know what *type* of thing a given sequence of bits is defined to be in order to know how to do computations on it.
...
This is also why R (and some other computer languages) treat `TRUE` and the number 1 and `FALSE` and the number 0 as equivalent in many contexts.
These are just 'on' or 'off' states of simple electronic components that make up our computers.

### (Optional aside) What sort of computations?

You might wonder what sort of computations this sort of simple representation using 1's and 0's permits.
...
The `is.numeric()` command tells R to the inputs are 'numeric', that is are numbers, and the `is.character()` command tells it to report whether the input contains a set of characters.
